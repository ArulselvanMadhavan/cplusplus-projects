...
void write_mpi_io_file(const char *filename, double **data, int data_size,
  ...
  MPI_File file_handle =
      create_mpi_io_file(filename, mpi_io_comm, (long long)data_size);

  MPI_File_set_view(file_handle, file_offset, MPI_DOUBLE, filespace, "native",
                    MPI_INFO_NULL);
  MPI_File_write_all(file_handle, &(data[0][0]), 1, memspace,
                     MPI_STATUS_IGNORE);
