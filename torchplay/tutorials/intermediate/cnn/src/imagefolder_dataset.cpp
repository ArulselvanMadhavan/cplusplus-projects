...
create_class_to_index_map(const std::vector<std::string> &classes) {
...
create_samples(const std::string &directory,
               const std::unordered_map<std::string, int> &class_to_idx) {
  std::vector<std::pair<std::string, int>> samples;

  for (const auto &[class_name, class_idx] : class_to_idx) {
    for (const auto &p : fs::directory_iterator(directory + "/" + class_name)) {
      if (p.is_regular_file()) {
        samples.emplace_back(p.path().string(), class_idx);
      }
    }
  }
...
ImageFolderDataset::ImageFolderDataset(const std::string &root, Mode mode,
...
torch::data::Example<> ImageFolderDataset::get(size_t index) {
  const auto &[image_path, class_idx] = samples_[index];
  return {load_image(image_path, image_load_size_), torch::tensor(class_idx)};
}
