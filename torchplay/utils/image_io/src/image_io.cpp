...
void norm_range(torch::Tensor t, const std::vector<double> &range) {
  if (range.empty()) {
    ...
    throw std::invalid_argument(
        "Range must either be empty or contain exactly 2 elements.");
  }
}
...
torch::Tensor make_grid(torch::Tensor tensor, int64_t nrow, int64_t padding,
                        ...
                        bool scale_each, torch::Scalar pad_value) {
  if (tensor.dim() == 2) {
  ...
  if (normalize) {
    if (scale_each) {
      ...
      for_each(mini_batches.begin(), mini_batches.end(),
               [&range](auto &t) { norm_range(t, range); });
    } else {
...
void save_image(torch::Tensor tensor, const std::string &file_path,
                int64_t nrow, int64_t padding, bool normalize,
                const std::vector<double> &range, bool scale_each,
                torch::Scalar pad_value, ImageFormat format) {
  auto grid =
      make_grid(tensor, nrow, padding, normalize, range, scale_each, pad_value)
          .mul(255)
          .add_(0.5)
          .clamp_(0, 255)
          .permute({1, 2, 0})
          .to(torch::kCPU, torch::kUInt8, false, false,
  ...
  case ImageFormat::BMP:
    stbi_write_bmp(file_path.c_str(), grid.size(1), grid.size(0), grid.size(2),
                   grid.data_ptr());
    break;
  case ImageFormat::JPG:
    stbi_write_jpg(file_path.c_str(), grid.size(1), grid.size(0), grid.size(2),
                   grid.data_ptr(), 100);
    break;
  default:
...
torch::Tensor
load_image(const std::string &file_path, torch::IntArrayRef shape,
           std::function<torch::Tensor(torch::Tensor)> transform) {
  if (!shape.empty() && shape.size() != 1 && shape.size() != 2) {
    throw std::invalid_argument(
        "Shape must be empty or contain exactly one or two elements.");
  }
  ...
  std::unique_ptr<unsigned char, decltype(&stbi_image_free)> image_raw(
      stbi_load(file_path.c_str(), &width, &height, &depth, 0),
      &stbi_image_free);
  ...
  if (shape.empty()) {
    return transform(
        torch::from_blob(image_raw.get(), {height, width, depth}, torch::kUInt8)
            .clone()
            .to(torch::kFloat32)
            .permute({2, 0, 1})
            .div_(255));
  ...
  stbir_resize_uint8(image_raw.get(), width, height, 0,
                     image_resized_buffer.data(), new_width, new_height, 0,
                     depth);
